"""
LoaderNga.py
Load National Greenhouse Accounts emission factors from NgaFactors.csv
...
MATCHING:
    ...
    that are prefixes of the full NGA Fuel_Name in NgaFactors.csv.
...
class NGAFactorsByYear:
    ...
    Reads NgaFactors.csv and provides lookup methods for any fuel,
    scope, year and state combination.
    """

    def __init__(self, folder_path=None):
        """Load NgaFactors.csv from ./Data/ next to this module.
    ...
    def _load(self):
        """Load NgaFactors.csv and index by year."""
        csv_path = os.path.join(self.folder_path, 'NgaFactors.csv')
        if not os.path.exists(csv_path):
            raise FileNotFoundError(
                f"NgaFactors.csv not found at {csv_path}.  "
                f"Run UtilityNgaToCsv.py to generate it."
            )
    ...
    def get_factors_for_year(self, year, state='QLD'):
        """Get emission factors for a specific year (used by Tab4Nger.py).

        Returns dict of factors for display.  Raises ValueError if required
        factors are missing from NgaFactors.csv - no hardcoded fallbacks.
        """
        year = self._resolve_year(year)
        if year is None:
